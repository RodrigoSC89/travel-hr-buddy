...
export default async function handler(
  ...
  try {
    ...
    const resultado = Array.from(conformidadeMap.values());
    ...
    console.error("Erro ao gerar dados de conformidade:", error);
    res.status(500).json({ error: "Erro ao gerar dados de conformidade." });
  }
}
