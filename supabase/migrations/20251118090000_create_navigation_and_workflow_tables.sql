-- Migration: create navigation tracking, module access logging, feature flags, and workflow AI tables
-- Date: 2025-11-18

-- Navigation history tracks transitions between routes for personalization
...
create index if not exists navigation_history_user_id_idx on public.navigation_history (user_id);
...
create policy if not exists "navigation_history_owner_all"
  on public.navigation_history
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- Module access log captures which modules users open for analytics
...
create policy if not exists "module_access_log_owner_all"
  on public.module_access_log
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);

-- Feature flags with optional user / tenant targeting
...
-- Workflow AI suggestions accepted by users
...
create policy if not exists "workflow_ai_suggestions_write"
  on public.workflow_ai_suggestions
  for insert
  with check (auth.uid() is not null);

-- Smart workflow steps table powers workflow analytics
...
create policy if not exists "smart_workflow_steps_write"
  on public.smart_workflow_steps
  for insert
  with check (auth.uid() is not null);
