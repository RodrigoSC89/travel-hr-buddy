-- ✅ Update check_daily_cron_execution function to use cron_execution_logs table
...
COMMENT ON FUNCTION public.check_daily_cron_execution() IS 
  'Checks if the daily assistant report cron (send-daily-assistant-report) has executed successfully in the last 36 hours using the cron_execution_logs table. Returns status ''ok'' or ''warning'' with a descriptive message.';
