-- Create SQL function to check if daily cron has executed in the last 36 hours
-- This function is used by the monitor-cron-health Edge Function to detect failures
...
BEGIN
  -- Get the most recent successful execution from automated triggers
  SELECT MAX(sent_at) INTO last_execution_time
...
COMMENT ON FUNCTION public.check_daily_cron_execution() IS 
  'Checks if the daily assistant report cron (send-daily-assistant-report) has executed successfully in the last 36 hours. Returns status ''ok'' or ''error'' with a descriptive message.';
