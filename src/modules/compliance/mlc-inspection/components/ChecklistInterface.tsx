  ...
  const handleAddFinding = async (item: typeof MLC_CHECKLIST_ITEMS[0], compliance: boolean, severity?: string, corrective_action?: string) => {
    try {
      await mlcInspectionService.createFinding({
        ...
        compliance,
        severity: severity as "critical" | "major" | "minor" | "observation",
        corrective_action,
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {MLC_CHECKLIST_ITEMS.map((item, index) => {
            ...
            return (
              <Card key={item.regulation} className="border-l-4" style={{ borderLeftColor: existingFinding ? (existingFinding.compliance ? "hsl(var(--success))" : "hsl(var(--destructive))") : "hsl(var(--muted-foreground))" }}>
