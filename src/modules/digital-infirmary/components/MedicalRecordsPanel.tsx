    ...
    <div className="space-y-6">
      ...
      <Card><CardContent className="p-6">
        ...
        <div className="grid grid-cols-4 gap-4 mt-6">
          ...
          <div className="p-4 rounded-lg bg-muted/50"><p className="text-sm text-muted-foreground">Alergias</p><div className="flex flex-wrap gap-1 mt-1">{selectedCrew.allergies.length > 0 ? selectedCrew.allergies.map((a) => <Badge key={a} variant="destructive" className="text-xs">{a}</Badge>) : <span className="text-xs text-muted-foreground">Nenhuma registrada</span>}</div></div>
          <div className="p-4 rounded-lg bg-muted/50"><p className="text-sm text-muted-foreground">Condições Crônicas</p><div className="flex flex-wrap gap-1 mt-1">{selectedCrew.chronicConditions.length > 0 ? selectedCrew.chronicConditions.map((c) => <Badge key={c} variant="outline" className="text-xs">{c}</Badge>) : <span className="text-xs text-muted-foreground">Nenhuma</span>}</div></div>
