               ...
               {filteredReqs.map((req) => (
                 <Card key={req.id} className={`cursor-pointer transition-all hover:shadow-md ${selectedRequisition?.id === req.id ? "ring-2 ring-primary" : ""}`} onClick={() => setSelectedRequisition(req)}>
                   <CardContent className="p-4">
                     ...
                     {req.approvalHistory.length > 0 && <div className="mt-3 flex items-center gap-1">{req.approvalHistory.map((step, stepIdx) => (<React.Fragment key={`${step.approver}-${step.role}`}><div className={`h-6 w-6 rounded-full flex items-center justify-center ${step.status === "approved" ? "bg-success text-success-foreground" : step.status === "pending" ? "bg-warning text-warning-foreground" : "bg-muted"}`}>{step.status === "approved" ? <CheckCircle2 className="h-3 w-3" /> : <Clock className="h-3 w-3" />}</div>{stepIdx < req.approvalHistory.length - 1 && <ChevronRight className="h-3 w-3 text-muted-foreground" />}</React.Fragment>))}</div>}
                   </CardContent>
                 </Card>
               ))}
             </div>
