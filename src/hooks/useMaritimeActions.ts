...
export const useMaritimeActions = () => {
  ...
  const handleExport = useCallback(
    async (moduleName: string, data?: any) => {
      setIsLoading(true);
      try {
  ...
  const handleRefresh = useCallback(
    async (moduleName: string, callback?: () => Promise<void>) => {
      setIsLoading(true);
      try {
        if (callback) {
          await callback();
  ...
  const handleCreate = useCallback(
    async (itemName: string, callback?: () => Promise<void>) => {
      setIsLoading(true);
      try {
        if (callback) {
          await callback();
  ...
  const handleViewDetails = useCallback(
    (itemName: string, id?: string | number) => {
  ...
  const handleGenerateReport = useCallback(
    async (reportName: string, callback?: () => Promise<void>) => {
      setIsLoading(true);
      try {
        if (callback) {
          await callback();
