...
export const useOfflineStorage = (): UseOfflineStorageReturn => {
  ...
  const addPendingChange = useCallback(async (action: string, data: any) => {
    try {
      ...
      await store.add(offlineData);} catch (error) {
  ...
  const syncPendingChanges = useCallback(async () => {
    ...
    try {
      const pendingChanges = await getPendingChanges();const db = await initDB();
      const transaction = db.transaction([OFFLINE_STORE], "readwrite");
      const store = transaction.objectStore(OFFLINE_STORE);
      
      for (const change of pendingChanges) {
        try {
          // Simulate API sync - replace with actual API calls// Mark as synced
          change.synced = true;
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      if (event.data.type === "OFFLINE_SYNC_COMPLETE") {updateCacheSize();
      }
    };
