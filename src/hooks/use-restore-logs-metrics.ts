...
export function useRestoreLogsMetrics(logs: RestoreLog[]): RestoreLogsMetrics {
  return useMemo(() => {
    const now = new Date();
    const weekStart = startOfWeek(now, { weekStartsOn: 1 });
    const monthStart = startOfMonth(now);

    const thisWeek = logs.filter(log => new Date(log.restored_at) >= weekStart).length;
    const thisMonth = logs.filter(log => new Date(log.restored_at) >= monthStart).length;
