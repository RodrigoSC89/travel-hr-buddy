...
export function useSystemHubData() {
  ...
  const systemMetrics = {
    ...
    activeIntegrations: integrations.filter((i: any) => i.status === "active" || i.status === "connected").length,
    failedIntegrations: integrations.filter((i: any) => i.status === "error" || i.status === "failed").length,
    totalUsers: users.length,
    activeUsers: users.filter((u: any) => u.status === "active").length,
    activeSessions: sessions.length,
    recentLogs: accessLogs.filter((l: any) => {
      const logTime = new Date(l.timestamp);
      const hourAgo = new Date(Date.now() - 60 * 60 * 1000);
      return logTime > hourAgo;
    }).length,
    errorLogs: accessLogs.filter((l: any) => l.severity === "error" || l.result === "error").length,
    totalSettings: settings.length,
    systemHealth: integrations.filter((i: any) => i.status === "active" || i.status === "connected").length / Math.max(integrations.length, 1) * 100,
  };

  // Get logs by severity
  const getLogsBySeverity = (severity: string) => {
    return accessLogs.filter((l: any) => l.severity === severity);
  };

  // Get setting by key
  const getSettingByKey = (key: string) => {
    return settings.find((s: any) => s.setting_key === key);
  };
