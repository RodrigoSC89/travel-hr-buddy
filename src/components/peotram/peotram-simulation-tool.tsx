...
export const PeotramSimulationTool: React.FC = () => {
  ...
  const calculateSimulationResults = () => {
    ...
    const trainingImpact =
      (parameters.trainingInvestment.simulatedValue - parameters.trainingInvestment.currentValue) /
      10000;
    ...
    const maintenanceImpact =
      (parameters.maintenanceFrequency.currentValue -
        parameters.maintenanceFrequency.simulatedValue) /
      5;
    ...
    const auditImpact =
      (parameters.auditFrequency.currentValue - parameters.auditFrequency.simulatedValue) / 10;
    baseScore += auditImpact * 0.2;
    complianceBase += auditImpact * 0.4;

    // Calcular impacto da equipe
    const staffingImpact =
      (parameters.staffing.simulatedValue - parameters.staffing.currentValue) / 10;
    baseScore += staffingImpact * 0.3;
    complianceBase += staffingImpact * 0.2;

    // Calcular impacto da tecnologia
    const technologyImpact =
      (parameters.technologyInvestment.simulatedValue -
        parameters.technologyInvestment.currentValue) /
      50000;
    baseScore += technologyImpact * 0.5;
    complianceBase += technologyImpact * 0.3;
    costBase +=
      parameters.technologyInvestment.simulatedValue - parameters.technologyInvestment.currentValue;
      ...
      impacts: [
        {
          category: "Segurança",
          current: 85,
          projected: Math.min(85 + (overallScore - 75) * 0.4, 100),
          improvement: (overallScore - 75) * 0.4,
        },
        {
          category: "Conformidade",
          current: 78,
          projected: Math.min(78 + (complianceRate - 70) * 0.5, 100),
          improvement: (complianceRate - 70) * 0.5,
        },
        {
          category: "Eficiência",
          current: 82,
          projected: Math.min(82 + (overallScore - 75) * 0.3, 100),
          improvement: (overallScore - 75) * 0.3,
        },
      ],
    };
  };
  ...
  const generateRecommendations = (): string[] => {
    ...
    if (
      parameters.maintenanceFrequency.simulatedValue < parameters.maintenanceFrequency.currentValue
    ) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Ajuste os parâmetros para simular diferentes cenários</CardDescription>
          </CardHeader>
              ...
              <div key={key} className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {param.simulatedValue !== param.currentValue && (
                      ...
                      >
                        {param.simulatedValue > param.currentValue ? "+" : ""}
                        {(
                          ((param.simulatedValue - param.currentValue) / param.currentValue) *
                          100
                        ).toFixed(1)}
                        %
                      </Badge>
                ...
                <div className="px-3">
                  ...
                  <div className="flex justify-between text-xs text-muted-foreground mt-1">
                    <span>
                      {param.min.toLocaleString("pt-BR")} {param.unit}
                    </span>
                    <span className="font-medium">
                      Atual: {param.currentValue.toLocaleString("pt-BR")} {param.unit}
                    </span>
                    <span>
                      {param.max.toLocaleString("pt-BR")} {param.unit}
                    </span>
                  </div>
                </div>
              </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Impactos projetados das alterações propostas</CardDescription>
          </CardHeader>
          <CardContent>
            {currentScenario ? (
              <div className="space-y-6">
                ...
                <div className="flex items-center justify-between p-3 bg-muted/30 rounded-lg">
                  ...
                  <Badge
                    variant="outline"
                    className={getRiskColor(currentScenario.results.riskLevel)}
                  >
                    {currentScenario.results.riskLevel === "low"
                      ? "Baixo"
                      : currentScenario.results.riskLevel === "medium"
                        ? "Médio"
                        : currentScenario.results.riskLevel === "high"
                          ? "Alto"
                          : "Crítico"}
                  </Badge>
                </div>
                  ...
                  {currentScenario.results.impacts.map(impact => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span
                          className={`text-sm font-medium ${getImpactColor(impact.improvement)}`}
                        >
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    ...
                    <div className="font-bold">
                      R$ {currentScenario.results.estimatedCost.toLocaleString("pt-BR")}
                    </div>
                  </div>
                  <div>
                    <span className="text-muted-foreground">Prazo de Implementação:</span>
                    <div className="font-bold">
                      {currentScenario.results.implementationTime} dias
                    </div>
                  </div>
                </div>
      ...
      {savedScenarios.length > 0 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Compare diferentes estratégias e suas projeções</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-sm">
                        ...
                        <div>
                          ...
                          <Badge
                            variant="outline"
                            className={getRiskColor(scenario.results.riskLevel)}
                          >
                            {scenario.results.riskLevel === "low"
                              ? "Baixo"
                              : scenario.results.riskLevel === "medium"
                                ? "Médio"
                                : "Alto"}
                          </Badge>
                        </div>
