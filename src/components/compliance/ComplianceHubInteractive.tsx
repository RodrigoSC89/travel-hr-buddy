/**
 * Compliance Hub Interactive - PATCH 900
 * Full CRUD for audits, certifications, and regulatory tracking
 */
...
import { 
  ...
  ClipboardList, Award, FileText, TrendingUp
} from "lucide-react";
...
export const ComplianceHubInteractive: React.FC = () => {
  ...
  const [audits, setAudits] = useState<Audit[]>([
    ...
    {
      id: "AUD-003",
      title: "SOLAS Safety Inspection",
      type: "SOLAS",
      status: "in_progress",
      vesselName: "MV Northern Star",
      auditorName: "Carlos Silva",
  ...
  const handleCreateAudit = () => {
    if (!newAudit.title || !newAudit.vesselName || !newAudit.scheduledDate) {
  ...
  const handleStartAudit = (audit: Audit) => {
    setAudits(audits.map(a => 
      a.id === audit.id ? { ...a, status: "in_progress" as const } : a
    ));
    toast({ title: "Auditoria Iniciada", description: `${audit.title} está em andamento` });
  };
  ...
  const handleCompleteAudit = (audit: Audit) => {
    ...
    toast({ title: "Auditoria Concluída", description: `Score: ${score}%` });
  };

  const handleDeleteAudit = (id: string) => {
    setAudits(audits.filter(a => a.id !== id));
    toast({ title: "Auditoria Removida", description: "Registro deletado com sucesso" });
  };

  const handleCreateCertificate = () => {
    if (!newCert.name || !newCert.vesselName || !newCert.expiryDate) {
      toast({ title: "Erro", description: "Preencha todos os campos obrigatórios", variant: "destructive" });
      return;
    }
  ...
  const filteredAudits = audits.filter(a => 
    ...
    a.vesselName.toLowerCase().includes(searchTerm.toLowerCase())
  );
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Gestão de Auditorias e Certificações</p>
          </div>
        </div>
      </div>
                  ...
                  >
                        ...
                        {audit.findings > 0 && (
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {audit.criticalFindings > 0 && (
                              <Badge variant="destructive">{audit.criticalFindings} críticos</Badge>
                            )}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {audit.status === "in_progress" && (
                        <Button size="sm" onClick={() => handleCompleteAudit(audit)}>
                          Concluir
                        </Button>
                      )}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-4 text-sm text-muted-foreground mt-1">
                          ...
                          <span>🏛️ {cert.issuingAuthority}</span>
                          <span>📅 Expira: {cert.expiryDate}</span>
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={showAuditDialog} onOpenChange={setShowAuditDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Ex: ISM Annual Audit 2024"
              />
            </div>
            <div>
              <Label>Tipo</Label>
              <Select
                value={newAudit.type}
                onValueChange={(v) => setNewAudit({ ...newAudit, type: v as Audit["type"] })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div>
              ...
              <Input
                value={newAudit.vesselName}
                onChange={(e) => setNewAudit({ ...newAudit, vesselName: e.target.value })}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateAudit}>Criar Auditoria</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={showCertDialog} onOpenChange={setShowCertDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              ...
              <Input
                ...
                placeholder="Ex: Safety Management Certificate"
              />
            </div>
            <div>
              <Label>Tipo</Label>
              <Input
                ...
                placeholder="Ex: SMC, ISSC, MLC"
              />
            </div>
            <div>
              <Label>Embarcação</Label>
              <Input
                value={newCert.vesselName}
                onChange={(e) => setNewCert({ ...newCert, vesselName: e.target.value })}
                placeholder="Nome da embarcação"
              />
            </div>
            <div>
              ...
              <Input
                ...
                placeholder="Ex: Lloyd's Register"
              />
            </div>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateCertificate}>Registrar Certificado</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
