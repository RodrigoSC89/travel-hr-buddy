...
export const CertificateManager: React.FC<CertificateManagerProps> = ({ employee, onClose }) => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
    case "active":
      return <Badge className="bg-success text-success-foreground"><CheckCircle size={12} className="mr-1" />Ativo</Badge>;
    case "expiring_soon":
      return <Badge className="bg-warning text-warning-foreground"><Clock size={12} className="mr-1" />Expirando</Badge>;
    case "expired":
      return <Badge className="bg-destructive text-destructive-foreground"><AlertTriangle size={12} className="mr-1" />Expirado</Badge>;
    default:
