...
const PerformanceOptimizer = () => {
  ...
  const runOptimization = async (optimization: typeof optimizations[number]) => {
    ...
    // Execute optimization via Supabase
    const { error } = await supabase.from("ai_insights")
      .update({ status: "active" })
      .eq("id", String(optimization.id));
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitoramento e otimização automática do sistema
          </p>
        </div>
