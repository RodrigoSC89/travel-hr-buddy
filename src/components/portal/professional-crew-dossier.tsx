...
export const ProfessionalCrewDossier: React.FC = () => {
  ...
  const generateAIRecommendations = async () => {
    ...
    try {
      const { error } = await supabase.functions.invoke("generate-crew-recommendations", {
        body: { crew_id: selectedCrewId }
      });
  ...
  if (loading) {
    return (
      <ShipLoader size="lg" label="Carregando dossiê profissional..." className="min-h-[500px]" />
    );
  }
