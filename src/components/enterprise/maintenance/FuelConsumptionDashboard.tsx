...
export function FuelConsumptionDashboard() {
  ...
  const totalConsumed = fuelRecords.reduce((sum: number, r: any) => sum + (Number(r.quantity_mt) || 0), 0);
  const totalCost = fuelRecords.reduce((sum: number, r: any) => sum + (Number(r.total_cost) || 0), 0);
  const totalROB = vessels.reduce((sum: number, v: any) => sum + (Number(v.current_fuel_level) || 0), 0);
  const avgPrice = fuelRecords.length > 0
    ? fuelRecords.reduce((sum: number, r: any) => sum + (Number(r.price_per_mt) || 0), 0) / fuelRecords.length
    : 0;

  // Group by fuel type for chart
  const fuelByType: Record<string, number> = {};
  fuelRecords.forEach((r: any) => {
  ...
  const exportCSV = () => {
    ...
    const rows = fuelRecords.map((r: any) => [
      r.record_date, vesselMap.get(r.vessel_id) || "N/A", r.fuel_type, r.quantity_mt, r.price_per_mt, r.total_cost, r.bunkering_port || ""
    ]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">ROB Total</p><p className="text-2xl font-bold">{totalROB.toLocaleString()} t</p></div><Droplets className="h-6 w-6 text-blue-600" /></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Consumo Registrado</p><p className="text-2xl font-bold">{totalConsumed.toFixed(1)} MT</p></div><Fuel className="h-6 w-6 text-amber-600" /></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Preço Médio</p><p className="text-2xl font-bold">${avgPrice.toFixed(0)}/MT</p></div><Gauge className="h-6 w-6 text-green-600" /></div></CardContent></Card>
        <Card><CardContent className="pt-4"><div className="flex items-center justify-between"><div><p className="text-sm text-muted-foreground">Custo Total</p><p className="text-2xl font-bold">${(totalCost / 1000).toFixed(0)}K</p></div><DollarSign className="h-6 w-6 text-purple-600" /></div></CardContent></Card>
      </div>
                    ...
                    {robPct !== null && (
                      <div className="mb-3">
                        ...
                        <div className="h-2 bg-muted rounded-full overflow-hidden">
                          <div className={`h-full rounded-full ${robPct < 25 ? "bg-red-500" : robPct < 50 ? "bg-amber-500" : "bg-green-500"}`} style={{ width: `${robPct}%` }} />
                        </div>
                      </div>
                    )}
                  </CardContent>
