...
export function ESGEmissionsDashboard() {
  ...
  if (isLoading) return <div className="space-y-4"><div className="grid grid-cols-4 gap-4">{[...Array(4)].map((_, i) => <Skeleton key={`esg-skeleton-${i}`} className="h-24" />)}</div><Skeleton className="h-64" /></div>;
