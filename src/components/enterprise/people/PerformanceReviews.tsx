...
export function PerformanceReviews() {
    ...
    async function fetch() {
      ...
      if (data) {
        ...
        const avgScore = wellbeing.length > 0
          ? wellbeing.reduce((s: number, w: any) => s + (w.overall_score || 0), 0) / wellbeing.length
          : 4.0;
        ...
        setMember({
          ...
          reviews: wellbeing.slice(0, 4).map((w: any, i: number) => ({
            reviewer: i === 0 ? "Superior" : i === 1 ? "Par" : i === 2 ? "Subordinado" : "Auto",
            relationship: (["superior", "peer", "subordinate", "self"] as const)[i] || "self",
            score: Math.min((w.overall_score || 4) / 20, 5),
            date: w.assessment_date || w.created_at || "",
            feedback: w.notes || undefined,
          })),
          okrs: [],
          competencies: [
  ...
  const getRelationshipBadge = (r: string) => {
    switch (r) { case "superior": return <Badge className="bg-purple-500/10 text-purple-500">Superior</Badge>; case "peer": return <Badge className="bg-blue-500/10 text-blue-500">Par</Badge>; case "subordinate": return <Badge className="bg-green-500/10 text-green-500">Subordinado</Badge>; default: return <Badge variant="outline">Auto-avaliação</Badge>; }
  };
  const getTrendIcon = (t: string) => { if (t === "up") return <TrendingUp className="h-4 w-4 text-green-500" />; if (t === "down") return <TrendingDown className="h-4 w-4 text-red-500" />; return <div className="h-4 w-4 border-t-2 border-muted-foreground" />; };
  ...
  return (
    <div className="space-y-6">
      <Card><CardContent className="pt-6"><div className="flex items-center justify-between">
        ...
        <div className="text-right"><div className="flex items-center gap-2"><Star className="h-6 w-6 text-yellow-500 fill-yellow-500" /><span className="text-3xl font-bold">{averageScore.toFixed(1)}</span><span className="text-muted-foreground">/5.0</span></div><p className="text-sm text-muted-foreground">Baseado em {member.reviews.length} avaliações</p></div>
        ...
        <TabsContent value="reviews" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            {["superior", "peer", "subordinate", "self"].map(type => {
              ...
              return <Card key={type}><CardContent className="pt-6 text-center"><div className="flex items-center justify-center gap-1 mb-2"><Star className="h-5 w-5 text-yellow-500 fill-yellow-500" /><span className="text-2xl font-bold">{review?.score?.toFixed(1) || "-"}</span></div>{getRelationshipBadge(type)}</CardContent></Card>;
            })}
          </div>
