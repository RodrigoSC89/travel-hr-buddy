...
export const FlightSearch = () => {
        ...
        const transformedFlights = data.data.data.map((offer: any, index: number) => {
          ...
          return {
            ...
            departure: {
              ...
              time:
                offer.itineraries[0]?.segments[0]?.departure?.at?.split("T")[1]?.substring(0, 5) ||
                "00:00",
            ...
            arrival: {
              ...
              time:
                offer.itineraries[0]?.segments[0]?.arrival?.at?.split("T")[1]?.substring(0, 5) ||
                "00:00",
              city: searchParams.to.split("(")[0].trim(),
              date:
                offer.itineraries[0]?.segments[0]?.arrival?.at?.split("T")[0] ||
                searchParams.departure,
            },
            duration:
  ...
  const stats = [
    {
      ...
      value:
        filteredFlights.length > 0
          ? `R$ ${Math.min(...filteredFlights.map(f => f.price))}`
          : "R$ 0",
      icon: DollarSign,
    ...
    },
    {
      ...
      value:
        filteredFlights.length > 0
          ? `R$ ${Math.round(filteredFlights.reduce((acc, f) => acc + (f.savings || 0), 0) / filteredFlights.length)}`
          : "R$ 0",
      icon: Plane,
    ...
    },
    {
      ...
      value:
        filteredFlights.length > 0
          ? Math.max(...filteredFlights.map(f => f.rating)).toFixed(1)
          : "0.0",
      icon: Star,
  ...
  const generateFlightBookingUrl = (offer: any, searchParams: any) => {
    ...
    const departureDate =
      offer.itineraries[0]?.segments[0]?.departure?.at?.split("T")[0] || searchParams.departure;
  ...
  const generateMockFlightBookingUrl = (flight: FlightOption, searchParams: any) => {
    ...
    return (
      flight.bookingUrl ||
      `https://www.kayak.com.br/flights/${originCode}-${destinationCode}/${departureDate}/${searchParams.passengers}adults`
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Busque e compare os melhores preços</p>
        </div>
      </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  <Badge variant="secondary">
                    ...
                    {Math.abs(predictions.predicted_price - predictions.current_avg_price).toFixed(
                      0
                    )}
                  </Badge>
                </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between">
              ...
              <div className="mt-4 lg:mt-0 lg:ml-8 text-center lg:text-right">
                <div className="space-y-2 mb-4">
                  ...
                  <p className="text-3xl font-bold text-success">R$ {flight.price}</p>
                  {flight.savings && (
      ...
      {filteredFlights.length === 0 && (
        <Card className="p-12 text-center">
          ...
          <p className="text-muted-foreground">Tente ajustar os parâmetros de busca</p>
        </Card>
      )}
    </div>
  );
