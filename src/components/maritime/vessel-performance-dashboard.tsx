  ...
  useEffect(() => {
    if (vesselData && vesselData.length > 0) {
      const transformedVessels: VesselDetails[] = vesselData.map((v, index) => ({
        ...
        performance: {
          fuelEfficiency: 80 + ((v.id || '').split("").reduce((a: number, c: string) => a + c.charCodeAt(0), 0) % 15),
          averageSpeed: 14 + (index * 1.7) % 6,
          uptime: 90 + (index * 3) % 8,
          maintenanceScore: 85 + (index * 7) % 10,
          safetyScore: 92 + (index * 5) % 7,
          emissions: 70 + (index * 11) % 20
        },
