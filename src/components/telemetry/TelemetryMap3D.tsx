      ...
      if (data) {
        const telemetryData: VesselTelemetry[] = data.map((v, idx) => {
          ...
          return {
            id: v.id,
            name: v.name,
            latitude: baseLatitude + ((idx * 7 % 20) - 10) * 0.1,
            longitude: baseLongitude + ((idx * 11 % 20) - 10) * 0.1,
            status: (["active", "active", "active", "idle", "maintenance", "active"][idx % 6] as VesselTelemetry["status"]) || "active",
            speed: 8 + (idx * 3) % 14,
            heading: (idx * 47) % 360,
            fuelLevel: 40 + (idx * 11) % 55,
            engineTemp: 75 + (idx * 3) % 20,
            hullStress: 10 + (idx * 7) % 30,
            lastUpdate: new Date().toISOString(),
            crewCount: 10 + (idx * 5) % 25,
            destination: ["Porto de Santos", "Rio de Janeiro", "Salvador", "Recife", "Manaus"][idx % 5],
            eta: new Date(Date.now() + (idx + 1) * 86400000).toISOString(),
            sensors: [
              { id: `temp-${v.id}`, type: "temperature", value: 75 + (idx * 3) % 20, unit: "°C", status: "normal", trend: "stable" },
              { id: `press-${v.id}`, type: "pressure", value: 1.0 + (idx * 0.05) % 0.3, unit: "bar", status: "normal", trend: "up" },
              { id: `vib-${v.id}`, type: "vibration", value: (idx * 0.7) % 5, unit: "mm/s", status: idx % 5 === 0 ? "warning" : "normal", trend: "stable" },
              { id: `fuel-${v.id}`, type: "fuel", value: 40 + (idx * 11) % 55, unit: "%", status: idx % 10 === 0 ? "warning" : "normal", trend: "down" },
            ]
          };
        });
