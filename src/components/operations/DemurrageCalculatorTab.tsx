              ...
              <tbody className="divide-y">
                {timeEntries.map(entry => (
                  <tr key={entry.id} className={entry.excluded ? "bg-muted/30 opacity-70" : ""}>
                    ...
                    <td className="p-2"><Button variant="ghost" size="icon" className="h-6 w-6" aria-label="Remover entrada" onClick={() => removeEntry(entry.id)}><Trash2 className="h-3 w-3" /></Button></td>
                  </tr>
                ))}
              </tbody>
            </table>
