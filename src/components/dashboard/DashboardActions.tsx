  ...
  const handleExport = useCallback(async (type: string) => {
    ...
    try {
      ...
      if (type === "csv") {
        ...
        const rows = [
          ...(vessels || []).map((v) => ["Embarcação", v.name || "-", v.status || "-", v.updated_at || "-"]),
          ...(crew || []).map((c) => ["Tripulante", c.full_name || "-", c.status || "-", c.updated_at || "-"]),
          ...(checklists || []).map((c) => ["Checklist", c.title || "-", c.status || "-", c.updated_at || "-"])
        ];
