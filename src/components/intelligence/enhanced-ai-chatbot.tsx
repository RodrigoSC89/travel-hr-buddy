...
const EnhancedAIChatbot: React.FC = () => {
  ...
  const quickCommands = [
    ...
    {
      text: "💡 Sugestões para reduzir custos operacionais",
  ...
  const handleSendMessage = async () => {
    ...
    ); // Variação realística no tempo de resposta
  };
          ...
          >
            ...
            <div className="relative z-10">
              ...
              <p className="text-lg opacity-95 mb-8 max-w-3xl drop-shadow-md font-medium">
                Sistema de IA mais avançado para operações marítimas com processamento de linguagem
                natural, análise preditiva e automação completa.
              </p>
              ...
              <Card className="bg-gradient-to-br from-card via-card/95 to-primary/5 hover:shadow-2xl transition-all duration-700 backdrop-blur-sm border border-border/50 h-[700px] flex flex-col">
                <CardHeader className="border-b border-border/50">
                  ...
                  <CardDescription className="flex items-center gap-2">
                    ...
                    <Badge
                      variant="outline"
                      className={`bg-${getCapabilityColor(aiCapabilities.find(c => c.id === selectedCapability)!)}/10 text-${getCapabilityColor(aiCapabilities.find(c => c.id === selectedCapability)!)} border-${getCapabilityColor(aiCapabilities.find(c => c.id === selectedCapability)!)}/30`}
                    >
                    ...
                    >
                      {messages.map((message, index) => (
                        ...
                        >
                          ...
                          <div
                            className={`max-w-2xl px-6 py-4 rounded-2xl shadow-lg transition-all duration-300 hover:shadow-xl ${
                              message.type === "user"
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Button
                                ...
                                aria-label={
                                  isVoiceActive ? "Parar gravação de voz" : "Iniciar comando de voz"
                                }
                              >
                                {isVoiceActive ? (
