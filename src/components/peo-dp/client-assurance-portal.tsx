...
import {
  Shield, Ship, CheckCircle, AlertTriangle, Clock, FileText, Download, Eye,
  Activity, Anchor, Users, Calendar, TrendingUp, Lock, Globe, BarChart3, RefreshCw
} from "lucide-react";
...
const fallbackVessel: VesselStatus = {
  id: "VSL-001", name: "MV Atlantic Explorer", dpClass: "DP-2", currentMode: "Auto DP",
  position: { lat: -22.9068, lon: -43.1729 }, heading: 145, operationalStatus: "green",
  complianceScore: 94, lastAudit: "2024-11-15", nextAudit: "2025-02-15", activeCrew: 3, asogStatus: "within_limits"
};
...
export const ClientAssurancePortal: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case "compliant": return <Badge className="bg-success text-success-foreground">Conforme</Badge>;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Badge variant="outline" className="flex items-center gap-1">
            <Lock className="h-3 w-3" />Acesso Controlado
          </Badge>
          <Button variant="outline" size="sm"><Download className="h-4 w-4 mr-2" />Relatório</Button>
        </div>
      </div>
        ...
        <CardContent className="pt-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Badge className={vessel.operationalStatus === "green" ? "bg-success text-success-foreground" : vessel.operationalStatus === "yellow" ? "bg-warning text-warning-foreground" : "bg-destructive text-destructive-foreground"}>
            ...
            <div className="text-center">
              ...
              <Badge variant="outline" className="mt-1"><Activity className="h-3 w-3 mr-1" />Ativo</Badge>
            </div>

            <div className="text-center">
              <p className="text-sm text-muted-foreground">Compliance Score</p>
              <p className={`text-3xl font-bold ${vessel.complianceScore >= 90 ? "text-success" : vessel.complianceScore >= 70 ? "text-warning" : "text-destructive"}`}>
            ...
            <div className="text-center">
              ...
              <div className="flex items-center justify-center gap-2 mt-2">
                {vessel.asogStatus === "within_limits" ? (
                  <><CheckCircle className="h-6 w-6 text-success" /><span className="font-medium text-success">Dentro dos Limites</span></>
                ) : vessel.asogStatus === "approaching_limits" ? (
                  <><AlertTriangle className="h-6 w-6 text-warning" /><span className="font-medium text-warning">Próximo ao Limite</span></>
                ) : (
                  <><AlertTriangle className="h-6 w-6 text-destructive" /><span className="font-medium text-destructive">Limite Excedido</span></>
                )}
              </div>
            </div>
          </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="overview" className="space-y-4">
            ...
            <Card>
              ...
              <CardContent>
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-xl font-bold">{new Date(vessel.lastAudit).toLocaleDateString("pt-BR")}</p><p className="text-sm text-muted-foreground">PEO-DP Completo</p></div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Próxima Auditoria</CardTitle></CardHeader>
              <CardContent>
                <div className="flex items-center gap-3">
                  <Clock className="h-8 w-8 text-warning" />
                  <div><p className="text-xl font-bold">{new Date(vessel.nextAudit).toLocaleDateString("pt-BR")}</p><p className="text-sm text-muted-foreground">Em 73 dias</p></div>
                </div>
              </CardContent>
            </Card>
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Equipe DP Ativa</CardTitle></CardHeader>
              <CardContent>
                <div className="flex items-center gap-3">
                  <Users className="h-8 w-8 text-success" />
                  <div><p className="text-xl font-bold">{vessel.activeCrew} DPOs</p><p className="text-sm text-muted-foreground">Todos certificados</p></div>
                </div>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Anchor className="h-5 w-5" />Posição Atual</CardTitle></CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div><p className="text-sm text-muted-foreground">Latitude</p><p className="text-lg font-mono">{vessel.position.lat.toFixed(4)}°</p></div>
                <div><p className="text-sm text-muted-foreground">Longitude</p><p className="text-lg font-mono">{vessel.position.lon.toFixed(4)}°</p></div>
                <div><p className="text-sm text-muted-foreground">Heading</p><p className="text-lg font-mono">{vessel.heading}°</p></div>
              </div>
            </CardContent>
          </Card>
        </TabsContent>
        ...
        <TabsContent value="compliance" className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            ...
            <Card className="bg-info/5 border-info/20">
              <CardContent className="pt-4 text-center">
                <TrendingUp className="h-8 w-8 text-info mx-auto" />
          ...
          <Card>
            <CardHeader><CardTitle>Itens de Compliance</CardTitle><CardDescription>Status detalhado de conformidade</CardDescription></CardHeader>
            <CardContent>
        ...
        <TabsContent value="operations" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="flex items-center gap-2"><Activity className="h-5 w-5" />Eventos Operacionais (Últimas 24h)</CardTitle></CardHeader>
            <CardContent>
              <ScrollArea className="h-[400px]">
                <div className="space-y-3">
                  {events.map((event) => (
                    <div key={event.id} className={`p-3 rounded-lg border-l-4 ${event.severity === "critical" ? "border-l-destructive bg-destructive/5" : event.severity === "warning" ? "border-l-warning bg-warning/5" : "border-l-info bg-info/5"}`}>
                      <div className="flex items-start justify-between">
                        <div>
                          <div className="flex items-center gap-2">
                            <Badge variant="outline">{event.type}</Badge>
                            <span className="text-xs text-muted-foreground">{new Date(event.timestamp).toLocaleString("pt-BR")}</span>
                          </div>
        ...
        <TabsContent value="documents" className="space-y-4">
          <Card>
            <CardHeader><CardTitle>Documentos Disponíveis</CardTitle><CardDescription>Documentação de compliance e operacional</CardDescription></CardHeader>
            <CardContent>
              <div className="space-y-2">
                {[
                  ...
                  { name: "DP Annual Trial Report", date: "2024-09-20", type: "PDF" },
                  { name: "Class DP Certificate", date: "2024-08-10", type: "PDF" },
                ].map((doc) => (
                  <div key={doc.name} className="flex items-center justify-between p-3 rounded-lg border">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Atualizado: {doc.date} • {doc.type}</p>
                      </div>
                    </div>
