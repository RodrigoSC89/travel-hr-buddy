...
export const TenantProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const loadTenantData = async () => {
    try {
      ...
      logger.logCaughtError("Error loading tenant data", err);
  ...
  const loadTenantBranding = async (tenantId: string) => {
    try {
      ...
      if (error && error.code !== "PGRST116") {
        logger.warn("Error loading tenant branding", { error, tenantId });
      }
      ...
      logger.warn("Could not load tenant branding", { error: err, tenantId });
    }
  };
  ...
  const loadCurrentTenantUser = async (tenantId: string) => {
    try {
      ...
      if (error && error.code !== "PGRST116") {
        logger.warn("Error loading tenant user", { error, tenantId, userId: user?.id });
      }
      ...
      logger.warn("Could not load tenant user", { error: err, tenantId, userId: user?.id });
    }
  };
  ...
  const loadTenantUsage = async (tenantId: string) => {
    try {
      ...
      if (error && error.code !== "PGRST116") {
        logger.warn("Error loading tenant usage", { error, tenantId });
      }
      ...
      logger.warn("Could not load tenant usage", { error: err, tenantId });
    }
  };
