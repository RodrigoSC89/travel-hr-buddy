        ...
        {threatData && (
          <div className="mt-4 p-4 bg-muted rounded-lg">
            ...
            <ul className="text-xs space-y-1">
              ...
              <li>Alertas Críticos: {threatData.threats.filter((t) => t.severity === "critical").length}</li>
              <li>Watchdog Alerts: {threatData.watchdogAlerts.length}</li>
              <li>Scores Ajustados: {threatData.scores.filter((s) => s.wasAdjusted).length}</li>
              <li>Score Médio: {(threatData.scores.reduce((sum, s) => sum + s.adjustedScore, 0) / threatData.scores.length).toFixed(1)}</li>
            </ul>
          </div>
        )}
      </CardContent>
