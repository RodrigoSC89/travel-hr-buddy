...
const forceUpdateIfNeeded = async () => {
  ...
  const CURRENT_VERSION = 'v45-zero-loading-blocks'; // PATCH v45: Zero loading blocks em todos os hooks
  ...
  try {
    ...
    if (storedVersion !== CURRENT_VERSION) {
      logger.info('[Boot v45] Version mismatch, forcing full cleanup...');
      console.log('[Boot v45] Cleaning caches and service workers...');
      ...
      if ('caches' in window) {
        try {
          ...
          console.log('[Boot v45] Caches cleared:', keys.length);
        } catch (e) {
          console.warn('[Boot v45] Cache clear failed:', e);
        }
      }
      ...
      if ('serviceWorker' in navigator) {
        try {
          ...
          console.log('[Boot v45] Service workers unregistered:', registrations.length);
        } catch (e) {
          console.warn('[Boot v45] SW unregister failed:', e);
        }
      }
      
      localStorage.setItem(SW_VERSION_KEY, CURRENT_VERSION);
      console.log('[Boot v45] Cleanup complete, version stored:', CURRENT_VERSION);
    }
  } catch (e) {
    logger.warn('[Boot v45] Error during update check', e instanceof Error ? { message: e.message } : undefined);
  }
};
...
const initServiceWorker = async () => {
  ...
  try {
    ...
    console.log('[Boot v45] SW registered', { scope: registration.scope });
  } catch (error) {
    console.warn('[Boot v45] SW registration failed (non-critical)', error);
  }
};
