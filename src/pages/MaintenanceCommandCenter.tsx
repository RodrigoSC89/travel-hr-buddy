/**
 * Maintenance Command Center - Optimized with Framer Motion + React.memo
 */
...
const StatCard = memo(({ label, value, icon: Icon, color, borderColor }: { label: string; value: string | number; icon: React.ElementType; color: string; borderColor: string }) => (
  <motion.div variants={kpiCard}>
    <Card className={`border-l-4 hover:shadow-md transition-all duration-300 ${borderColor}`}>
      <CardContent className="pt-4">
        <div className="flex items-center justify-between">
          ...
          <Icon className={`h-8 w-8 text-${color} opacity-50`} />
        </div>
      </CardContent>
    </Card>
  </motion.div>
...
const MaintenanceCommandCenter = () => {
  ...
  const handleExportWeeklySchedule = useCallback(async () => {
    toast({ title: "Exportação", description: "Gerando relatório semanal de manutenção..." });
  }, [toast]);

  const handleShowAlerts = useCallback(() => setShowAlertsPanel(true), []);
  const handleShowCreate = useCallback(() => setShowCreateDialog(true), []);
    ...
    <motion.div initial="hidden" animate="visible" variants={staggerContainer} className="container mx-auto p-6 space-y-6">
      ...
      <motion.div variants={fadeUp} className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <Button onClick={handleShowCreate} className="bg-gradient-to-r from-warning to-warning/80 hover:from-warning/90 hover:to-warning/70"><Plus className="mr-2 h-4 w-4" />Novo Plano</Button>
        </div>
      </motion.div>
      ...
      <motion.div variants={staggerContainer} initial="hidden" animate="visible" className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {statItems.map((s) => (
          <StatCard key={s.label} label={s.label} value={s.value} icon={s.icon} color={s.color} borderColor={s.borderColor} />
        ))}
      </motion.div>
      ...
      <motion.div variants={fadeUp}>
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          ...
          <TabsContent value="jobs" className="mt-6"><JobsCenter onCreateJob={handleShowCreate} /></TabsContent>
