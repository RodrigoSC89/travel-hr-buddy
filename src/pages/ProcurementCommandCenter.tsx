/**
 * PROCUREMENT COMMAND CENTER - Orchestrator
 * Refactored: sub-components in src/pages/procurement/
 */
...
import { 
  ShoppingCart, Plus, FileText, RefreshCw, DollarSign,
  AlertTriangle, Users, Send, Zap
} from "lucide-react";
import { cn } from "@/lib/utils";
import { useToast } from "@/hooks/use-toast";
import { 
  type Supplier, type RFQRequest, type StockItem, 
  type PurchaseRecommendation, type InventoryItem 
...
export default function ProcurementCommandCenter() {
  ...
  const filteredSuppliers = suppliers.filter(s => s.company_name.toLowerCase().includes(searchQuery.toLowerCase()) || s.city?.toLowerCase().includes(searchQuery.toLowerCase()) || s.country?.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <div className="min-h-screen bg-background p-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
          {[
            ...
            <Card key={i} className={`${kpi.gradient ? `bg-gradient-to-br ${kpi.gradient}` : 'bg-card/50 backdrop-blur'} ${kpi.borderColor}`}>
              <CardContent className="p-4">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${kpi.iconColor === 'text-primary' ? 'bg-primary/20' : kpi.iconColor === 'text-success' ? 'bg-success/20' : kpi.iconColor === 'text-warning' ? 'bg-warning/20' : 'bg-secondary/20'}`}>
        ...
        <ProcurementTabs
          ...
          onViewAlternatives={(rec) => { setSelectedRecommendation(rec); setShowAlternativesDialog(true); }}
          onShowNewRFQDialog={() => setShowNewRFQDialog(true)}
          onShowNewItemDialog={() => setShowNewItemDialog(true)}
          onCopyItem={(item) => { navigator.clipboard.writeText(`Item: ${item.name} | Estoque: ${item.current_stock}/${item.minimum_stock} | Valor: R$ ${item.total_value?.toFixed(2) || '0.00'}`); toast({ title: "✏️ Dados copiados", description: "Dados do item copiados para clipboard." }); }}
        />
