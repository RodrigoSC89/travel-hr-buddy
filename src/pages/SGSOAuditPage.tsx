...
export default function SGSOAuditPage() {
  ...
  const handleExportPDF = () => {
    const element = document.getElementById("sgso-audit-content");
    if (element) {
      html2pdf().from(element).save("auditoria-sgso.pdf");
    }
  };

  return (
    <div className="container mx-auto p-6 max-w-5xl">
      <h1 className="text-3xl font-bold mb-6">🛡️ Auditoria SGSO - IBAMA</h1>
      <div id="sgso-audit-content" className="space-y-4">
        {auditData.map((item, idx) => (
          <Card key={item.num} className="border p-4">
            <CardContent className="space-y-2">
              ...
              <Textarea
                placeholder="📄 Evidência observada"
                value={item.evidence}
      ...
      <div className="flex gap-4 pt-6">
        <Button onClick={handleSubmit}>📤 Enviar Auditoria</Button>
