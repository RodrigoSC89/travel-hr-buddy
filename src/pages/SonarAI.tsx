    ...
    <div className="p-6 space-y-6">
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? 'Editar' : 'Novo'} Modelo IA</DialogTitle></DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <select value={form.type} onChange={e => setForm({ ...form, type: e.target.value as Enhancement['type'] })} className="w-full h-10 px-3 border rounded-md"><option value="noise_reduction">Redução de Ruído</option><option value="detection">Detecção</option><option value="classification">Classificação</option></select>
