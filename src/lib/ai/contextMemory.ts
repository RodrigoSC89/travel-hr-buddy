...
export async function getContextSnapshot(contextId: string = 'global'): Promise<SystemContextSnapshot> {
  try {
    ...
    logger.error('Error fetching context snapshot:', error);
    throw error;
  }
}
...
export async function updateContextSnapshot(snapshot: Partial<SystemContextSnapshot> & { contextId: string }) {
  try {
    ...
    if (error) {
      logger.error('Error updating context snapshot:', error);
      throw error;
    }

  } catch (error) {
    logger.error('Error in updateContextSnapshot:', error);
    throw error;
  }
}
...
export async function addRecentEvent(
  ...
  try {
    ...
    logger.error('Error adding recent event:', error);
  }
}
