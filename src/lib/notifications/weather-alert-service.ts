...
export async function sendWeatherAlertToBackend(
  ...
  try {
    ...
    if (error) {
      logger.error("[WeatherAlert] Failed to send to backend:", error);
      Sentry.captureException(error, {
        tags: { component: "weather-alert-service" },
      });
    }
  } catch (err) {
    logger.error("[WeatherAlert] Backend notification error:", err);
  }
}
...
export async function storeWeatherAlert(
  ...
  try {
    ...
    logger.error("[WeatherAlert] Failed to store alert:", err);
  }
}
