/**
 * Pre-OVID Realtime Audio - WebRTC connection for OpenAI Realtime API
 * Enables hands-free inspection with voice commands
 */
...
export class PreOVIDRealtimeChat {
  ...
  async init() {
    try {
      ...
      const { data, error } = await supabase.functions.invoke("realtime-voice-session", {
        body: { 
          vesselType: this.options.vesselType,
  ...
  private handleMessage(event: RealtimeMessage) {
    ...
    switch (event.type) {
      ...
      case 'conversation.item.input_audio_transcription.completed':
        if (typeof event.transcript === 'string') {
          this.options.onTranscript(event.transcript, true);
        }
        break;
