...
export async function testBookingConnection(): Promise<BookingTestResult> {
  ...
  if (!bookingService.isConfigured()) {
    return {
      ...
      error: 'Missing VITE_BOOKING_API_KEY. Note: Booking.com typically requires RapidAPI subscription or partnership approval.',
    };
  }

  try {
    // Test with city suggestions - lightweight endpoint
