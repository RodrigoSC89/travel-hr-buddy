...
const getSimilarCases = async (embedding: number[], matchThreshold = 0.7, matchCount = 5): Promise<SimilarCase[]> => {
  try {
    ...
    if (error) {
      logger.warn("Error fetching similar cases from database:", error);
      return [];
    }
    ...
    logger.warn("Database not available, using mock similar cases");
    return [
...
export const getCopilotSuggestions = async (
  ...
  try {
    ...
    logger.error("Error fetching copilot suggestions:", error);
    throw error;
  }
};
...
export const streamCopilotSuggestions = async (
  ...
  try {
    ...
    logger.error("Error streaming copilot suggestions:", error);
    throw error;
  }
};
